...
private:
    ...
    void printValue(const Sentence* sentence)
    {
        ...
        size_t frontSize = descSize == 1 ? 0 : descSize % 2 ? descSize / 2 - 1 : descSize / 2;

        std::cout << std::string(frontSize, ' ') << sentence->evaluate() << std::string(descSize - frontSize + 1, ' ');
    }
    ...
    bool checkTable(std::vector<int> symbols, size_t index)
    {
        ...
        for (const Sentence* sentence : knowledgeModel->atomicSentences)
        {
            ...
            case -1:
                break;
            default:
                throw std::logic_error("Unexpected value detected");
            }
        }
...
public:
    TruthTable(const KnowledgeModel& knowledgeModel) : knowledgeModel(&knowledgeModel), count(0) { }
