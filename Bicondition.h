...
public:
    ...
    Bicondition(const Sentence* s1, const Sentence* s2) :
        Symbol(
            ...
            + ((s1->getDescription() < s2->getDescription() ? s2 : s1)->isSymbol() ? ")" : "")),
        sentences{
            s1->getDescription() < s2->getDescription() ? s1 : s2,
            s1->getDescription() < s2->getDescription() ? s2 : s1}
    {
        if (!s1 || !s2)
        {
    ...
    bool evaluate() const override { return sentences[0]->evaluate() == sentences[1]->evaluate(); }

    size_t atomicCount() const override { return sentences[0]->atomicCount() + sentences[1]->atomicCount(); }
    ...
    {
        return (sentences[0] == other.sentences[0] && sentences[1] == other.sentences[1]);
    }
};
