...
private:
	...
	void checkPremise(const Sentence* currentSentence)
	{
		for (const Sentence* sentence : knowledgeModel->compoundSentences)
		{
			...
			{
				std::cout << sentence->getDescription() << " does not contain " 
					<< currentSentence->getDescription() << "\n";
				continue;
			}
		
			if (--placeholder->premiseCount) 
			{
				std::cout << placeholder->getDescription() << " premise is set to " 
					<< placeholder->premiseCount << "\n";
				continue;
			}
...
public:
	ForwardChaining(const KnowledgeModel& knowledgeModel) :
	...
	bool execute()
	{
		...
		return std::find(inferredSentences.begin(), inferredSentences.end(),
			knowledgeModel->query) != inferredSentences.end();
	}
};