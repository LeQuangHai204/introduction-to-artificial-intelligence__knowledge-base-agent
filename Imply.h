...
public:
    ...
    Imply(const Sentence* antecedent, const Sentence* consequence) :
        Symbol(
            (antecedent->isSymbol() ? "(" : "") + antecedent->getDescription() + (antecedent->isSymbol() ? ")" : "")
            + "=>" + (consequence->isSymbol() ? "(" : "") + consequence->getDescription()
    ...
    bool evaluate() const override { return !antecedent->evaluate() || consequence->evaluate(); }

    size_t atomicCount() const override { return antecedent->atomicCount() + consequence->atomicCount(); }
    ...
    {
        return antecedent == other.antecedent && consequence == other.consequence;
    }
};
